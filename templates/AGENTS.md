...
**本部分引用 `dev-workflow/multi-agent-safety` skill**

AI 助手在涉及 Git 操作时，**必须遵守**以下约定：

#### [强制] 推送确认流程

执行 `git push` 前必须：
1. **检查用户指令**：用户是否明确说了"推送"、"push"、"提交到远程"？
2. **如无明确指令**：必须询问 "是否推送到远程仓库？"
3. **等待明确确认**：仅当用户明确回答 [是/确认/推送] 后才可执行
...
- [X] `git stash`（创建或应用）
- [X] `git checkout`（切换或创建分支，除非用户明确要求）
- [X] `git reset`、`git rebase`、`git cherry-pick`
- [X] `git add -A` 或 `git add .`
- [X] `git commit -am`（自动添加所有文件）
- [X] `git push -f` 或强制推送
- [X] 直接 push 到 main/master 分支（未经明确允许）

#### [禁止] 擅自扩展

- [X] **不要**擅自创建用户未要求的文件、脚本、配置
- [X] **不要**添加超出用户明确指令范围的功能
- [X] 严格执行"最小修改原则"
