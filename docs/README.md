# Vibe Coding 方法论文档

本目录包含 Vibe Coding（AI 协作开发）方法论的完整文档。

## 文档索引

| 文档 | 内容 |
|------|------|
| [overview.md](overview.md) | Vibe Coding 概述与核心概念 |
| [architecture.md](architecture.md) | 五层架构详解 |
| [methodology.md](methodology.md) | 四种核心方法论 |
| [multi-agent.md](multi-agent.md) | 多代理安全规则 |
| [skill-design.md](skill-design.md) | 技能库设计原则 |
| [workflow.md](workflow.md) | 实施流程与最佳实践 |
| [ glossary.md](glossary.md) | 术语表 |

## 快速开始

如果你是第一次接触 Vibe Coding，建议按以下顺序阅读：

1. [overview.md](overview.md) - 了解基本概念
2. [architecture.md](architecture.md) - 理解系统架构
3. [workflow.md](workflow.md) - 掌握实施流程
4. [skill-design.md](skill-design.md) - 学习技能设计

---

*基于 OpenClaw 项目实践总结*
